...
export default function Chats() {
	...
	async function onSendMessage() {
		...
		const dataToSendToWSS = JSON.stringify({
			...
			body: { chatID: chatID.current, message: message },
		});
		ws.current.send(dataToSendToWSS);
		setMessage("");
	}
						...
						chatsList.map((chat) => {
							return (
								<div
									...
									className="m-2 rounded-lg shadow-md drop-shadow-md flex flex-row justify-evenly bg-white h-fit w-full"
								>
									{" "}
									{/* A single chat */}
					...
					>
						{messagesList.length == 0 ? (
							<div className="flex flex-col items-center justify-center h-full">
								...
								<h2 className="text-xl font-normal">
									{" "}
									Or, select a chat from the left!{" "}
								</h2>
							</div>
						) : (
