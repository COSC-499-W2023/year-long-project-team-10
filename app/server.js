...
wss.on("connection", (ws, req) => {
	...
	sessionMiddleWare(req, {}, () => {
		...
		// CHECK SESSION HERE EACH MESSAGE
	...
	ws.on("message", (message) => {
		...
		console.log("MESSAGELOG: ", action, body);
		// let chatID;
		// if (action !== "signedIn") chatID = body.chatID;
		req.body = body;
		(async () => {
			switch (action) {
				case "insertMessage":
				...
				case "deleteMessage":
					...
					break;
				case "signedIn":
					console.log("RECEIVED SIGN IN"); // Corrected logging message
					ws.send(
						JSON.stringify({
							type: "userStatus",
							status: "signedIn",
